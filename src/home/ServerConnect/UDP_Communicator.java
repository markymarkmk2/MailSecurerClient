...
public class UDP_Communicator
{
    ...
    private int ping;
    final static String udp_mtx = new String("");
    ...
    public int ping( String id, int delay_ms )
    {
        ...
        synchronized (this)
        {
            answer = udp_send(str, false, main.do_scan_local() ? "localhost" : null, 0, null, delay_ms);
            ok = check_answer();
        }
        if (ok)
        {
    ...
    public String udp_send( String str, boolean scan, String dest_ip, int retries )
    {
        return udp_send(str, scan, dest_ip, retries, null);
    }

    public boolean send_fast_retry_cmd( String str )
    {
        return send_cmd( str, 2500 );
    }
    public boolean send_fast_retry_cmd( StationEntry ste, String str )
    {
        return send_cmd( ste, str, 2500 );
    }

    public boolean send_cmd( String string, int to )
    {
        return send_cmd(string, null, to);
    }
    public boolean send_cmd(  StationEntry ste, String string, int to )
    {
        return send_cmd(ste, string, null, to);
    }

    public boolean send_cmd( String string, OutputStream outp, int to )
    {
        boolean ok = false;
        ...
        if (ste != null)
        {
            ok = send_cmd(ste, string, outp, to);
        }
        return ok;
    }
    public boolean send_cmd( StationEntry ste, String string, OutputStream outp, int to )
    {
        String id = Long.toString(ste.get_id()) + ":";

        synchronized (this)
        {
            answer = udp_send(id + string, false, ste.get_ip(), 0, null, to);
    ...
    public String udp_send( String str, boolean scan, String dest_ip, int retries, OutputStream outp )
    {
        return udp_send(str, scan, dest_ip, retries, outp, -1);
    }

    public String udp_send( String str, boolean scan, String dest_ip, int retries, OutputStream outp, int timeout )
    {
        UserMain.debug_msg(4, "udp_send: " + str + " SC:" + (scan ? "1" : "0"));
        DatagramSocket keep_udp_s = null;

        synchronized (udp_mtx)
        {
            try
            {
                if (dest_ip == null)
...
//                    keep_udp_s = new DatagramSocket(UDP_CLIENT_PORT, InetAddress.getByName("localhost"));
                ...
                {
                    out_packet = new DatagramPacket(out_data, out_data.length, InetAddress.getByName(dest_ip), UDP_SERVER_PORT);
                }
                ...
                try
                {
                    ...
                    if (!scan)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                UserMain.debug_msg(4, "udp_recv: " + result);
                            }
                        }
