...
public class EditHotfolder extends GlossDialogPanel
{
    ...
    public EditHotfolder(int _row, HotfolderOverview _overview)
    {
        ...
        model = vbox_overview.get_hf_model();
        ...
        if (!model.is_new(row))
        {
            hf = (Hotfolder)model.getSqlResult().getObject(_row);

            TXT_PATH.setText(hf.getPath());
            CB_MAILACCOUNT.addItem( hf.getUsermailadress() );
            CB_VAULT.removeAllItems();
            BT_DISABLED.setSelected( hf_is_disabled() );
        }
        else
        {
            hf = new Hotfolder();
    ...
    int get_hf_flags()
    {
        ...
        try
        {
            flags = Integer.parseInt(hf.getFlags());
        }
        catch (NumberFormatException numberFormatException)
        {
            Logger.getLogger("").log(Level.SEVERE, "Invalid flag for Hotfolder: " + hf.getFlags());
        }

        return flags;
    }

    boolean hf_is_disabled()
    {
        int flags = 0;
    ...
    void set_hf_disabled(boolean f)
    {
        int flags = get_hf_flags();
        if (f)
        {
            flags |= HotfolderOverview.HF_DISABLED;
        }
        else
        {
            flags &= ~HotfolderOverview.HF_DISABLED;
        }
        hf.setFlags(Integer.toString(flags));
    }
    ...
    private boolean check_changed()
    {
        ...
        if (TXT_PATH.getText().compareTo(hf.getPath() ) != 0)
            return true;   
        
        if (BT_DISABLED.isSelected() != hf_is_disabled())
            return true;
    ...
    private boolean is_plausible()
    {
        ...
        try
        {
            String email = CB_MAILACCOUNT.getSelectedItem().toString();
    ...
    private boolean is_vaild_email(String sEmail)
    {        
        try
        {
    ...
    private boolean update_db()
    {
        ...
        String name = TXT_PATH.getText();
        String email = CB_MAILACCOUNT.getSelectedItem().toString();
        boolean de = BT_DISABLED.isSelected();

        hf.setPath(name);
        
        hf.setUsermailadress(email);
        set_hf_disabled( de );
